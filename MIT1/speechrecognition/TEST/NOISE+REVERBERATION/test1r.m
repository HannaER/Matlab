...
%VÄNSTER
decibel_v = 0;
load TEST_REC/r1/v.mat
for i = 1:P % all words to iterate through
    temp1 = [];
    temp2 = [];
    temp3 = [];
    temp4 = [];
    for j = 1:length(rec1vr(1,1).ch1)
        if  abs(rec1vr(1,i).ch3(j)) > 0.0005 % this value is chosen by looking at recordings
            temp1 = [temp1 rec1vr(1,i).ch1(j)];
            temp2 = [temp2 rec1vr(1,i).ch2(j)];
            temp3 = [temp3 rec1vr(1,i).ch3(j)];
            temp4 = [temp4 rec1vr(1,i).ch4(j)];
        end
    end
...
%HÖGER
decibel_h = 0;
load TEST_REC/r1/h.mat
for i = 1:P % all words to iterate through
    temp1 = [];
    temp2 = [];
    temp3 = [];
    temp4 = [];
    for j = 1:length(rec1hr(1,1).ch1)
        if  abs(rec1hr(1,i).ch3(j)) > 0.0005 % this value is chosen by looking at recordings
            temp1 = [temp1 rec1hr(1,i).ch1(j)];
            temp2 = [temp2 rec1hr(1,i).ch2(j)];
            temp3 = [temp3 rec1hr(1,i).ch3(j)];
            temp4 = [temp4 rec1hr(1,i).ch4(j)];
        end
    end
    variance_ = mean([var(temp1) var(temp2) var(temp3) var(temp4)]);
...
index = exceptions(1);
ch1=rec1vr(1,index).ch1;
ch2=rec1vr(1,index).ch2;
ch3=rec1vr(1,index).ch3;
ch4=rec1vr(1,index).ch4;
index = exceptions(2);
ch1= ch1 + rec1hr(1,index).ch1;
ch2= ch2 + rec1hr(1,index).ch2;
ch3= ch3 + rec1hr(1,index).ch3;
...
%     if i <= N/2
%         ch1_w = [ch1_w rec1vr(1,index_w).ch1'];
    ...
    for i = 1:M % M = antal brusnivåer
        ...
        for j = 1:N  % N = antal ord
            ...
            if j <= N/2 % take 'vänster'
                ...
                ch3 = rec1vr(1,index).ch3;
                ch4 = rec1vr(1,index).ch4;
                current_word_name = rec1vr(1).name;
            else % take 'höger'
                ch1 = rec1hr(1,index).ch1;
                ch2 = rec1hr(1,index).ch2;
                ch3 = rec1hr(1,index).ch3;
                ch4 = rec1hr(1,index).ch4;
                current_word_name = rec1hr(1).name;
            end
            w.ch1 = ch1;
            w.ch2 = ch2;
            w.ch3 = ch3;
...
save('TEST\NOISE+REVERBERATION\result1r.mat', 'result1r','-v7.3');
display('plotting');
...
figure (1)
subplot(2,2,1)
y = extractfield(result1r.result11, 'wer');
plot(snr1,y);
...
figure (2)
y = extractfield(result1r.result11mbf, 'wer');
