...
for i = 1:length(cases.list)
    ...
    if(j == 1)
        ...
        figure('Name', strcat(names{i}, ': Errors'));
        resultCase = strcat('result', num2str(j), '1mbf');
        y = extractfield(setup.(resultCase), 'deletion');
        plot(snr1, y, 'k-o', 'LineWidth', 1.5);
        hold on;
        resultCase = strcat('result', num2str(j), '1mbf');
        y = extractfield(setup.(resultCase), 'no_match');
        plot(snr1, y, 'r-o', 'LineWidth', 1.5);
        resultCase = strcat('result', num2str(j), '1mbf');
        y = extractfield(setup.(resultCase), 'wrong_word');
        plot(snr1, y, 'b-o', 'LineWidth', 1.5);        
        resultCase = strcat('result', num2str(j), '4');
        y = extractfield(setup.(resultCase), 'deletion');
        plot(snr1,y, 'k-p', 'LineWidth', 1.5);
        resultCase = strcat('result', num2str(j), '4');
        y = extractfield(setup.(resultCase), 'no_match');
        plot(snr1,y, 'r-p', 'LineWidth', 1.5);
        resultCase = strcat('result', num2str(j), '4');
        y = extractfield(setup.(resultCase), 'wrong_word');
        plot(snr1,y, 'b-p', 'LineWidth', 1.5);
        title(strcat(num2str(j), ' meter: Noise: Errors'), 'FontSize', 16);
        l11 = legend(  '1 mic - bf: deletion', '1 mic - bf: no match', '1 mic - bf: wrong word', '4 mic + bf: deletion', '4 mic + bf: no match', '4 mic + bf: wrong word', 'Location', 'NorthEast');
        xlabel('SNR [dB]', 'FontSize', 16);
...
for i = 1:length(casesr.list)
    ...
    if(j == 1)
        title(strcat(num2str(j), ' meter'), 'FontSize', 16);
        figure('Name', strcat(namesr{i}, ': Errors'));
        resultCase = strcat('result', num2str(j), '1mbf');
        y = extractfield(setup.(resultCase), 'deletion');
        plot(snr1, y, 'k-o', 'LineWidth', 1.5);
        hold on;
        ...
        y = extractfield(setup.(resultCase), 'wrong_word');
        plot(snr1,y, 'b-p', 'LineWidth', 1.5);
        title(strcat(num2str(j), ' meter: Noise + Reverberation: Errors'), 'FontSize', 16);
        l11 = legend(  '1 mic - bf: deletion', '1 mic - bf: no match', '1 mic - bf: wrong word', '4 mic + bf: deletion', '4 mic + bf: no match', '4 mic + bf: wrong word', 'Location', 'NorthEast');
        xlabel('SNR [dB]', 'FontSize', 16);
        ylabel('Error [%]', 'FontSize', 16);
        set(gca, 'fontsize', 12);
...
for i = 1:length(casesvad.list)
    ...
    if(j == 1)
        title(strcat(num2str(j), ' meter'), 'FontSize', 16);
        figure('Name', strcat(namesvad{i}, ': Errors'));
        resultCase = strcat('result', num2str(j), '1mbf');
        y = extractfield(setup.(resultCase), 'deletion');
        plot(snrvad, y, 'k-o', 'LineWidth', 1.5);
        hold on;
        resultCase = strcat('result', num2str(j), '1mbf');
        y = extractfield(setup.(resultCase), 'no_match');
        plot(snrvad, y, 'r-o', 'LineWidth', 1.5);
        resultCase = strcat('result', num2str(j), '1mbf');
        y = extractfield(setup.(resultCase), 'wrong_word');
        plot(snrvad, y, 'b-o', 'LineWidth', 1.5);        
        resultCase = strcat('result', num2str(j), '4');
        y = extractfield(setup.(resultCase), 'deletion');
        plot(snrvad,y, 'k-p', 'LineWidth', 1.5);
        resultCase = strcat('result', num2str(j), '4');
        y = extractfield(setup.(resultCase), 'no_match');
        plot(snrvad,y, 'r-p', 'LineWidth', 1.5);
        resultCase = strcat('result', num2str(j), '4');
        y = extractfield(setup.(resultCase), 'wrong_word');
        plot(snrvad,y, 'b-p', 'LineWidth', 1.5);
        title(strcat(num2str(j), ' meter: Real-Time Simulation: Errors'), 'FontSize', 16);
        l11 = legend(  '1 mic - bf: deletion', '1 mic - bf: no match', '1 mic - bf: wrong word', '4 mic + bf: deletion', '4 mic + bf: no match', '4 mic + bf: wrong word', 'Location', 'NorthEast');
        xlabel('SNR [dB]', 'FontSize', 16);
        ylabel('Error [%]', 'FontSize', 16);
        set(gca, 'fontsize', 12);
...
%     
%     if(j == 1)
%         title(strcat(num2str(j), ' meter: Errors: Deletions'), 'FontSize', 16);
%     else
%         title(strcat(num2str(j), ' meters: Errors: Deletions'), 'FontSize', 16);
%     end
%     l4 = legend( '1 mic   + noise - bf', '1 mic   + noise + bf','2 mics + noise + bf','3 mics + noise + bf','4 mics + noise + bf', 'Location', 'NorthWest');
%     xlabel('SNR [dB]', 'FontSize', 16);
